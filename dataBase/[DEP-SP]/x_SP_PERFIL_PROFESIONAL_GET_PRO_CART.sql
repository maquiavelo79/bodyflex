...
-- CALL SP_PERFIL_PROFESIONAL_GET_PRO_CART('tqghogcvkhl7b70riagdq3ne14');

DROP PROCEDURE IF EXISTS bodyflex.SP_PERFIL_PROFESIONAL_GET_PRO_CART;
CREATE PROCEDURE bodyflex.`SP_PERFIL_PROFESIONAL_GET_PRO_CART`(
                                                                IN se VARCHAR(50)
                                                              )
BEGIN
...
-- SELECT * FROM CARRO
  ...
  SELECT *
  FROM CARRO C, CARRO_DETALLE CD 
  WHERE C.CAID=CD.CAID AND C.CASESION ='hvfhqmrsdl5b0mhpu2m8plsnl2';
