...
-- CALL SP_CP_ADM_CSU_POS_DET(0,0,@codErr);
-- SELECT @codErr AS corErr;

DROP PROCEDURE IF EXISTS bodyflex.SP_CP_ADM_CSU_POS_DET;
CREATE PROCEDURE bodyflex.`SP_CP_ADM_CSU_POS_DET`(
                                                      IN sw INTEGER
...
BEGIN
  ...
  -- CANTIDAD DE PRODUCTOS
    SET @CANT=(SELECT COUNT(*) FROM POSTULACION WHERE POSEST=3);
      ...
      CASE
        WHEN sw=0 THEN -- PRIMERA LLAMADA       
          IF(ULTIMO<>0)THEN  
            ...
            , ULTIMOS AS 'ULT'
            FROM POSTULACION 
            WHERE POSID<=ULTIMO AND POSEST=3
            ORDER BY POSID DESC
            LIMIT 10;
          ELSE
            SELECT POSID
