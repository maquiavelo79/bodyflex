CALL SP_CP_ADM_CSU_PRO_COL2(0,0,0,0,@codErr);
SELECT @codErr;

CALL SP_CP_ADM_CSU_PRO_COL2(1,1,0,0,@codErr);
SELECT @codErr;

CALL SP_CP_ADM_CSU_PRO_COL2(1,1,4,0,@codErr);
SELECT @codErr;

CALL SP_CP_ADM_CSU_PRO_COL2(2,0,0,0,@codErr);
SELECT @codErr;

CALL SP_CP_ADM_CSU_PRO_COL2(3,0,0,0,@codErr);
SELECT @codErr;

CALL SP_CP_ADM_CSU_PRO_COL2(0,0,0,12,@codErr);
SELECT @codErr;
...
DROP PROCEDURE IF EXISTS bodyflex.SP_CP_ADM_CSU_PRO_COL2 ;
CREATE PROCEDURE bodyflex.`SP_CP_ADM_CSU_PRO_COL2`(
                                                    IN cat1 INTEGER
                                                    , IN cat2 INTEGER
                                                    , IN cat3 INTEGER
  ...
  -- EXISTENCIA DE PRODUCTOS
    ...
    IF(@CANT)>0 THEN
      ...
      IF(ULTIMO<>0)THEN  
        ...
        , IFNULL(IF(CHAR_LENGTH((SELECT cprId FROM COLECCION_PRODUCTO WHERE proId=P.proId LIMIT 1))>0,"SI","NO"),"NO") AS CAT
        FROM PRODUCTO P
        -- WHERE P.proId<id 
