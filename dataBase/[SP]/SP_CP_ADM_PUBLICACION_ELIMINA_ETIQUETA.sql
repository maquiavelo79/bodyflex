...
DROP PROCEDURE IF EXISTS bodyflex.SP_CP_ADM_PUBLICACION_ELIMINA_ETIQUETA;
CREATE PROCEDURE bodyflex.`SP_CP_ADM_PUBLICACION_ELIMINA_ETIQUETA`( 
                                                                IN rut VARCHAR(10)
                                                                , IN cat VARCHAR(50)
                                                                , IN eti VARCHAR(50)
                                                                , OUT codErr INTEGER
...
BEGIN
  ...
  IF EXISTS(SELECT * FROM PUBLICACION_LIST_ETIQUETAS WHERE ETRUT=rut AND CATETINOM=cat AND ETNOM=eti) THEN 
    IF NOT EXISTS(SELECT * FROM PUBLICACION_ETIQUETA PE, PUBLICACION P WHERE PE.PUID=P.PUID AND P.PUEST='INACTIVO' AND CATETINOM=cat AND ETNOM=eti) THEN
      ...
      END IF;
    ELSE
      SET codErr=96; -- NO ES POSIBLE ELIMINAR ETIQUETA ASOCIADA A UNA PUBLICACIÓN EN ESTADO DISTINTO DE 'INACTIVO'
    END IF;
  ELSE
    SET codErr=97; -- SOLO EL CREADOR DE LA ETIQUETA LA PUEDE ELIMINAR  
  END IF;
