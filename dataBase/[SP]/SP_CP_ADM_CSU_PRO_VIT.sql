...
-- CALL SP_CP_ADM_CSU_PRO_VIT(0,0,@codErr);
-- SELECT @codErr AS corErr;

DROP PROCEDURE IF EXISTS bodyflex.SP_CP_ADM_CSU_PRO_VIT ;
CREATE PROCEDURE bodyflex.`SP_CP_ADM_CSU_PRO_VIT`(
                                                     IN sw INTEGER
...
BEGIN
  ...
  -- CANTIDAD DE PRODUCTOS
    SET @CANT=(SELECT COUNT(*) FROM PRODUCTO WHERE proPE=1 AND proEnVi=1);
    ...
    IF(@CANT)>0 THEN
      ...
      WHILE CONT<PAG DO
        ...
        IF CONT=0 THEN            
          ...
          SELECT proId 
          FROM PRODUCTO
          WHERE proPE=1 AND proEnVi=1
          ORDER BY proId DESC LIMIT 10;                     
        ELSE
          CREATE TEMPORARY TABLE TMP_Pag1 
          SELECT proId
          FROM PRODUCTO 
          WHERE proId<id AND proPE=1 AND proEnVi=1  
          ORDER BY proId DESC LIMIT 10;
        END IF;
      ...
      CASE
        WHEN sw=0 THEN -- PRIMERA LLAMADA       
          IF(ULTIMO<>0)THEN  
            ...
            , p.proEt
            FROM PRODUCTO P
            WHERE P.proId<=ULTIMO AND P.proPE=1 AND proEnVi=1
