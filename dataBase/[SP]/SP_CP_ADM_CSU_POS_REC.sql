...
-- CALL SP_CP_ADM_CSU_POS_REC(0,0,@codErr);
-- SELECT @codErr AS corErr;

DROP PROCEDURE IF EXISTS bodyflex.SP_CP_ADM_CSU_POS_REC;
CREATE PROCEDURE bodyflex.`SP_CP_ADM_CSU_POS_REC`(
                                                      IN sw INTEGER
...
BEGIN
  ...
  -- CANTIDAD DE PRODUCTOS
    SET @CANT=(SELECT COUNT(*) FROM POSTULACION WHERE POSEST=5 OR POSEST=7);
    ...
    IF(@CANT)>0 THEN
      ...
      WHILE CONT<PAG DO
        IF CONT=0 THEN            
          ...
          SELECT POSID 
          FROM POSTULACION
          WHERE POSEST=5 OR POSEST=7
          ORDER BY POSID DESC LIMIT 10;                     
        ELSE
          CREATE TEMPORARY TABLE TMP_Pag1 
          SELECT POSID
          FROM POSTULACION 
          WHERE POSID<id AND POSEST=5 OR POSEST=7
          ORDER BY POSID DESC LIMIT 10;
        END IF;
      ...
      CASE
        WHEN sw=0 THEN -- PRIMERA LLAMADA       
          IF(ULTIMO<>0)THEN  
            ...
            , POSEST
            FROM POSTULACION 
            WHERE POSID<=ULTIMO AND (POSEST=5 OR POSEST=7)
            ORDER BY POSID DESC
            LIMIT 10;
          ELSE
            SELECT POSID
