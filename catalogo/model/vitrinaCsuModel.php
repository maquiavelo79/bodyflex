...
include("../model/conection.php");
    ...
    try{
        ...
        if($conn){   
            ...
            $sql="CALL SP_CAT_CSU_VIT(@codErr);";
            ...
            if($num>0){
                ...
                while ($r = $stmt->fetch(PDO::FETCH_NUM)):
                    ...
                    $item='item_'.$pId;
                    $producto='pro_'.$pId;
                    //$divIcoV='divIcoV_'.$pId;
                    $icoPV='icoPV_'.$pId;
                    
                    $sTr.='<div id="'.$item.'" class="item">';
                        $sTr.='<div id="'.$producto.'" class="product altura">';
                            $sTr.='<a class="add-fav tooltipHere" data-toggle="tooltip" data-original-title="Add to Wishlist" data-placement="left">';
                                $sTr.='<i id="'.$icoPV.'" class="glyphicon glyphicon-heart"></i>';
                            $sTr.='</a>';
                            $sTr.='<div class="image">';
                                $sTr.='<div class="quickview">';
                                $sTr.='<a id_quick="'.$pId.'" onclick="modalFunctionVitrina('.$pId.');" class="btn btn-xs btn-quickview">Quick View</a>';
                                $sTr.='</div>';
                ...
                switch($codErr){
                    ...
                    case 99:
                        ...
                        break;
                    case 98:
                        $desErr='SIN PRODUCTOS EN VITRINA';
                        break;
                }
            }        
        }else{
