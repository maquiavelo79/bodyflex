...
include("../model/conection.php");
    ...
    try{
        ...
        if($conn){    
            ...
            $sql="CALL SP_CP_ADM_CSU_CAT1(:ultimo, @codErr);";
            ...
            if($num>0){
                while ($r = $stmt->fetch(PDO::FETCH_NUM)): 
                    ...
                    $sTr = '<tr style="cursor:pointer;">';
                       $sTr.='<td style="width: 33%;" class="center">' . $cat1Id   . '</td>';
                       $sTr.='<td style="width: 33%;" class="center">' . $cat1Nom . '</td>';
                       $sTr.='<td style="width: 33%;" class="center">' . $cat1GD . '</td>';
                    $sTr.='</tr>';
                ...
                $paginacion.='<ul>';
                    for($i=1;$i<=$pag;$i++){
                        ...
                        if($pa==$i){
                            ...
                            $paginacion.='<li style="cursor:pointer;"><a onclick="consultaCategoria1(' . $ultimos[$j] . ',' . $i . ')">' . $i . '</a></li>';
                        }
                    }
                $paginacion.='</ul>';
