...
//sleep(1);
    ...
    try{
        ...
        if($conn){
            ...
            $sql="CALL SP_CP_ADM_MOD_EST_POS_ING_EVA(:id, @codErr);";    
            ...
            if($num>0){
                while ($r = $stmt->fetch(PDO::FETCH_NUM)):   
                    $sTrR.=$r[0];
                endwhile; 
                envioEmailEvaluacion($email, $nom, $ape);
            }else{
    ...
    function envioEmailEvaluacion($email, $nom, $ape){
        ...
        $mail->Subject = "Bodyflex - Evaluacion Postulacion";
        
            $template = file_get_contents('../plantilla/postulacionEvaluacion.html','r');
            $template = str_replace("%NOMBRE%",strtoupper($nom),$template);
            $template = str_replace("%APELLIDO%",strtoupper($ape),$template);
                        
        $mail->Body = $template;
        $mail->AltBody = "Estimado(a) " . $nom .' '. $ape. " Tu postulación se encuentra etapa de evauación, por favor ten a mano evidencias como: ";
