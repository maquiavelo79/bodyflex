...
include("../model/conection.php");
    ...
    try{
        ...
        if($conn){   
            ...
            $sql="CALL SP_CP_ADM_CSU_POS_REC(:sw, :ultimo, @codErr);";
                ...
                while ($r = $stmt->fetch(PDO::FETCH_NUM)):
                    ...
                    $sTr = '<tr style="cursor:pointer;" class="registro">';
                        ...
                        if($posEst==5){
                            ...
                            $sTr.='</td>';  
                            $sTr.='<td class="center">';
                                $sTr.='<span id='.$posId.' nom='.$posNom.' ape='.$posApe.' email='.$posEma.' style="text-align: center; font-weight: bold; color: white; cursor: pointer;" class="label label-error">RECHAZAR</span>';
                            $sTr.='</td>'; 
                        }elseif($posEst==7){
                            $sTr.='<td colspan="2" class="center">';
                                $sTr.='<span id='.$posId.' style="font-weight: bold; color: white;" class="btn btn-small btn-danger">RECHAZADA</span>';
                            $sTr.='</td>';  
