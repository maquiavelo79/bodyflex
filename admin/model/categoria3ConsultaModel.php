...
include("../model/conection.php");
    ...
    try{
        ...
        if($conn){    
            ...
            $sql="CALL SP_CP_ADM_CSU_CAT3(:cat1, :cat2, :ultimo, @codErr);";
            ...
            if($num>0){
                while ($r = $stmt->fetch(PDO::FETCH_NUM)): 
                    ...
                    $sTr = '<tr style="cursor:pointer;">';
                       ...
                       $sTr.='<td style="width: 20%;" class="center">' . $cat2Id . '</td>';
                       $sTr.='<td style="width: 20%;" class="center">' . $cat2Nom  . '</td>';
                       $sTr.='<td style="width: 20%; font-weight: bold;" class="center">' . $cat3Id . '</td>';
                       $sTr.='<td style="width: 20%;" class="center">' . $cat3Nom  . '</td>';
                       $sTr.='<td style="width: 20%;" class="center">' . $cat3GD  . '</td>';
                       $sTr.='<td style="width: 20%;" class="center">' . $cat3GD2  . '</td>';
                    $sTr.='</tr>';
                    ...
                    $cant=$r[7];
                    $pag=$r[8];
                    $paginaciones=$r[9]; //ultimo de cada paginacion
                ...
                $paginacion.='<ul>';
                    for($i=1;$i<=$pag;$i++){
                        ...
                        if($pa==$i){
                            ...
                            $paginacion.='<li style="cursor:pointer;"><a onclick="consultaCategoria3('. $cat1 . ',' . $cat2 . ',' . $ultimos[$j] . ',' . $i . ')">' . $i . '</a></li>';
                        }
                    }
                $paginacion.='</ul>';
