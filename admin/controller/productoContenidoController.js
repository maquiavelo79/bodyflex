...
var URLprotocol = window.location.protocol;
    ...
    $(document).on("click", "#btnAgrCon", function(event){
        ...
        var proId = $('#txtProId').val().trim(); 
        var tipCon = $('#cmbTipConPro').val().trim(); 
        var IdDri = $('#txtIdDrivePro').val().trim(); 
        var idCon = $('#txtIdConPro').val().trim(); // ID de contenido ingresada si existe //
        //var ipr = 0;
    ...
    $(document).on("click", "#btnEliCon", function(event){
        ...
        $.ajax({
                data:  parametros,
                url: URLprotocol+"//"+URLdomain+"/bodyflex/admin/model/productoEliminaContenidoModel.php",
                type:  'post',
                datetype: 'xml',
                beforeSend: function(){
                    $("#modalBody").html(strLoad);
