...
include("../model/conection.php");
    ...
    try{
        ...
        if($conn){   
            ...
            $sql="CALL SP_CP_PRO_PUBLICACION_INGRESA_MODIFICA_CONTENIDO(:puId, :tipCon, :url, :idCon, @codErr);";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':puId', $puId, PDO::PARAM_STR,100);
            $stmt->bindParam(':tipCon', $tipCon, PDO::PARAM_STR,20);
            $stmt->bindParam(':url', $url, PDO::PARAM_STR,200);
            $stmt->bindParam(':idCon', $idCon, PDO::PARAM_STR,100);
            $stmt->execute();
            $num= $stmt->rowCount();
